...
import {query, InfoById, downloadExcel} from '../../../services/trade/withdraw/withdrawOrder'
...
export default modelExtend(pageModel, {
  ...
  subscriptions: {
    setup({dispatch, history}) {
      history.listen((location) => {
        if (location.pathname === '/withdrawOrder') {
          dispatch({
            type: 'query',
  ...
  effects: {
    ...
    * InfoView({payload}, {select, call, put}) {
      const id = yield select(({withdrawOrder}) => withdrawOrder.currentItem.id)
